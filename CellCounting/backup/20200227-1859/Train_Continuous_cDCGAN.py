...
def train_Continuous_cDCGAN(train_labels, kernel_sigma, epoch_GAN, dim_GAN, trainloader, netG, netD, optimizerG, optimizerD, save_images_folder, save_models_folder = None, ResumeEpoch = 0, device="cuda", shift_label = 0, max_label = 1):
    ...
    # def kernel_weights_all(yjs, yis, epsilon): # for fixed y_i and epsilon
    #     numerator_vec = np.exp(-(ys-y_i)**2/(2*kernel_sigma**2)) * np.exp((ys-y_i)*epsilon/(kernel_sigma**2)) * np.exp(-epsilon**2/(2*kernel_sigma**2))
    #     denominator = np.sum(exp_y_diff_square_dict[y_i] * (exp_y_diff_dict[y_i]**epsilon) * np.exp(-epsilon**2/(2*kernel_sigma**2)))
    #     return numerator_vec/denominator
    ...
    for epoch in range(ResumeEpoch, epoch_GAN):
        for batch_idx, (batch_train_images, batch_train_labels) in enumerate(trainloader):
            ...
            if batch_idx%20 == 0:
                print ("cDCGAN: [Iter %d] [Epoch %d/%d] [D loss: %.4f] [G loss: %.4f]" % (gen_iterations, epoch + 1, epoch_GAN, d_loss.item(), g_loss.item()))
