...
def train_Continuous_cDCGAN(train_labels, kernel_sigma, threshold_type, epoch_GAN, dim_GAN, trainloader, netG, netD, optimizerG, optimizerD, save_images_folder, save_models_folder = None, ResumeEpoch = 0, device="cuda", tfboard_writer=None):
    ...
    for epoch in range(ResumeEpoch, epoch_GAN):
        for batch_idx, (batch_train_images, batch_train_labels) in enumerate(trainloader):
            ...
            batch_fake_images_1 = netG(z, batch_train_labels_1)
            ...
            for j in range(BATCH_SIZE):
                if threshold_type == "soft":
                    ...
                    if np.abs(batch_train_labels_1[j].item()-batch_train_labels_2[j].item()-batch_epsilons[j]) < 2*kernel_sigma:
                        weights_x_j[j] = 1
            weights_x_j = torch.from_numpy(weights_x_j).type(torch.float).to(device)



            d_loss = - torch.mean(weights_x_j * torch.log(real_dis_out)) - torch.mean(weights_x_j * torch.log(1 - fake_dis_out)) #only add weights to real out
