...
if args.GAN in ["cGAN"]: #treated as classification; convert angles to class labels
    ...
    unique_labels = np.sort(np.array(list(set(labels_train)))).astype(np.int)
...
if args.comp_FID:
    ...
    if args.eval_mode == 1:
        center_start = np.min(labels_train)+args.FID_radius
        center_stop = np.max(labels_train)-args.FID_radius
    else:
...
eval_results_logging_fullpath = args.root_path + '/output/Eval_results.txt'
