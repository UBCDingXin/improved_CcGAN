...
class CcGAN_SAGAN_Discriminator(nn.Module):
    ...
    def forward(self, x, labels):
        # labels = self.label_encoder(labels)
        # 128x128
        out = self.opt_block1(x)  # 64 x 64
        out = self.block1(out)    # 32 x 32
        out = self.self_attn(out) # 32 x 32
        out = self.block2(out)    # 16 x 16
        out = self.block3(out)    # 8 x 8
        out = self.block4(out)    # 4 x 4
...
if __name__ == "__main__":
    ...
    netG = CcGAN_SAGAN_Generator(dim_z=256, dim_embed=128, gene_ch=64).cuda() #131367044 parameters
    netD = CcGAN_SAGAN_Discriminator(dim_embed=128, disc_ch=64).cuda() #162158402 parameters
