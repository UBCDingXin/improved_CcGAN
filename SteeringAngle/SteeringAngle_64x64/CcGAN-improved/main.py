...
if args.GAN == "CcGAN":
    ...
    trainloader_embed_net = torch.utils.data.DataLoader(trainset_embed, batch_size=args.batch_size_embed, shuffle=True) #use data before replication
...
if args.visualize_fake_images:
    ...
    if not os.path.isfile(filename_real_images):
        ...
        for i_row in range(n_row):
            ...
            #     label_lower = 0
            # else:
            #     label_lower = (displayed_normalized_labels[i_row-1] + displayed_normalized_labels[i_row])/2.0
            # if i_row == n_row-1:
            #     label_upper = 1.0
            # else:
            #     label_upper = (displayed_normalized_labels[i_row] + displayed_normalized_labels[i_row+1])/2.0
            # label_lower = label_lower*max_label_after_shift-np.abs(min_label_before_shift)
            # label_upper = label_upper*max_label_after_shift-np.abs(min_label_before_shift)
    ...
    # # First, visualize conditional generation; horizontal grid
    # ## 3 rows; 10 columns (3 samples for each label)
    # n_row = 3
    # n_col = 10
    # displayed_normalized_labels = np.linspace(0.05, 0.95, n_col)
    # ### output fake images from a trained GAN
    # if args.GAN == 'CcGAN':
    #     filename_fake_images = save_images_folder + '/{}_{}_sigma_{}_kappa_{}_fake_images_grid_{}x{}.png'.format(args.GAN, args.threshold_type, args.kernel_sigma, args.kappa, n_row, n_col)
    # else:
    ...
    #     curr_label = displayed_normalized_labels[i_col]
    #     for j_row in range(n_row):
    #         curr_image, _ = fn_sampleGAN_given_label(1, curr_label, 1)
    #         images_show[j_row*n_col+i_col,:,:,:] = curr_image
    # images_show = torch.from_numpy(images_show)
    # save_image(images_show.data, filename_fake_images, nrow=n_col, normalize=True)
    #
    # ### output some real images as baseline
    ...
    #     for i_col in range(n_col):
    #         # generate 3 real images from each interval
    #         label_lower = displayed_normalized_labels[i_col]*max_label_after_shift-np.abs(min_label_before_shift)-1
    #         label_upper = displayed_normalized_labels[i_col]*max_label_after_shift-np.abs(min_label_before_shift)+1
    #         for j_row in range(n_row):
    #             indx_curr_label = np.where((raw_labels>=label_lower)*(raw_labels<label_upper)==True)[0]
    #             np.random.shuffle(indx_curr_label)
    #             indx_curr_label = indx_curr_label[0]
    #             images_show[j_row*n_col+i_col] = raw_images[indx_curr_label]
    #     images_show = (images_show/255.0-0.5)/0.5
    #     images_show = torch.from_numpy(images_show)
    #     save_image(images_show.data, filename_real_images, nrow=n_col, normalize=True)
