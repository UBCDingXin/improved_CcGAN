...
class cont_cond_cnn_generator(nn.Module):
    def __init__(self, nz=128, img_size=64, dim_embed=DIM_EMBED):
        ...
        self.genblock0 = ResBlockGenerator(GEN_SIZE*8, GEN_SIZE*8, dim_embed=self.dim_embed) #4--->8
        self.genblock1 = ResBlockGenerator(GEN_SIZE*8, GEN_SIZE*4, dim_embed=self.dim_embed) #8--->16
        self.genblock2 = ResBlockGenerator(GEN_SIZE*4, GEN_SIZE*2, dim_embed=self.dim_embed) #16--->32
        self.genblock3 = ResBlockGenerator(GEN_SIZE*2, GEN_SIZE, dim_embed=self.dim_embed) #32--->64
...
class cont_cond_cnn_discriminator(nn.Module):
    def __init__(self, img_size=64, dim_embed=DIM_EMBED):
        ...
        # self.linear1 = nn.Linear(DISC_SIZE*8, 1, bias=True)
        # nn.init.xavier_uniform_(self.linear1.weight.data, 1.)
        # self.linear1 = spectral_norm(self.linear1)
        # self.linear2 = nn.Linear(self.dim_embed, DISC_SIZE*8, bias=False)
        # nn.init.xavier_uniform_(self.linear2.weight.data, 1.)
        # self.linear2 = spectral_norm(self.linear2)
