...
def train_cGAN(GAN_name, unique_labels, EPOCHS_GAN, GAN_Latent_Length, trainloader, netG, netD, optimizerG, optimizerD, criterion, save_cGANimages_folder, save_models_folder = None, ResumeEpoch = 0, device="cuda"):
    ...
    if save_models_folder is not None and ResumeEpoch>0:
        save_file = save_models_folder + "/{}_checkpoint_intrain/{}_checkpoint_epoch".format(GAN_name, GAN_name) + str(ResumeEpoch) + ".pth"
    ...
    for epoch in range(ResumeEpoch, EPOCHS_GAN):
        for batch_idx, (batch_train_images, batch_train_labels) in enumerate(trainloader):
            ...
            if batch_idx%20 == 0:
                print ("%s: [Iter %d] [Epoch %d/%d] [D loss: %.4f] [G loss: %.4f] [D prob real:%.4f] [D prob fake:%.4f]" % (GAN_name, gen_iterations, epoch + 1, EPOCHS_GAN, d_loss.item(), g_loss.item(), prob_real.mean().item(),prob_fake.mean().item()))
            ...
            if gen_iterations % 1000 == 0:
                ...
                save_image(gen_imgs.data, save_cGANimages_folder +'%d.png' % gen_iterations, nrow=n_row, normalize=True)

        if save_models_folder is not None and (epoch+1) % 1000 == 0:
            save_file = save_models_folder + "/{}_checkpoint_intrain/".format(GAN_name)
            if not os.path.exists(save_file):
                os.makedirs(save_file)
            save_file = save_file + "{}_checkpoint_epoch".format(GAN_name) + str(epoch+1) + ".pth"
            torch.save({
...
def SampcGAN(netG, class2label, GAN_Latent_Length = 128, NFAKE = 10000, batch_size = 500, nc=3, img_size=64, num_classes = 100, device="cuda"):
    '''
...
def SampcGAN_given_label(netG, given_label, unique_labels, label2class, nc=3, img_size=64, GAN_Latent_Length = 128, NFAKE = 10000, batch_size = 500, device="cuda"):
    '''
