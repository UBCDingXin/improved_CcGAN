...
def train_CcGAN(kernel_sigma, threshold_type, kappa, epoch_GAN, dim_GAN, trainloader, netG, netD, optimizerG, optimizerD, save_images_folder, save_models_folder = None, ResumeEpoch = 0, device="cuda", plot_in_train=False, samples_tar_eval = None, angle_grid_eval = None):
    '''
    ...
    for epoch in range(ResumeEpoch, epoch_GAN):
        for batch_idx, (batch_train_samples, batch_train_angles) in enumerate(trainloader):
            ...
            if threshold_type == "soft":
                ...
                real_weights_x_j = np.clip(np.exp(-kappa*(batch_train_angles_1.cpu().numpy()-batch_comb_labels_2.cpu().numpy())**2), 1e-20, 1e+20)
                fake_weights_x_j = np.clip(np.exp(-kappa*(batch_comb_labels_1.cpu().numpy()-batch_comb_labels_2.cpu().numpy())**2), 1e-20, 1e+20)
            else:
