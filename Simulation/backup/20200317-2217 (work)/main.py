...
# angles for plotting
unseen_angles_all = np.linspace(0, 2*np.pi, n_gaussians*10+1)
...
for nSim in range(args.nsim):
    ...
    # samples_plot_in_train, _, _ = generate_data(10, angle_grid_eval)
    ...
    if not os.path.isfile(filename_tmp):
        ...
        plt.grid(b=True)
        plt.scatter(samples_train[:, 0], samples_train[:, 1], c='blue', edgecolor='none', alpha=0.5, s=point_size)
        plt.scatter(means_train[:, 0], means_train[:, 1], c='red', edgecolor='none', alpha=1, s=point_size)
    ...
    if args.GAN == "cGAN": #treated as classification; convert angles to class labels
        ...
        if args.kappa < 0:
            if args.threshold_type=="hard":
                ...
                args.kappa = 1/(2*(2/args.n_gaussians)**2)
    ...
    if args.eval:
        ...
        plt.scatter(fake_samples[:, 0], fake_samples[:, 1], c='red', edgecolor='none', alpha=1, s=point_size, label="Fake samples")
        plt.legend()
        plt.savefig(filename_tmp)
