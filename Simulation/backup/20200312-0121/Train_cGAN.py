...
def train_cGAN(EPOCHS_GAN, GAN_Latent_Length, trainloader, netG, netD, optimizerG, optimizerD, criterion, save_models_folder = None, ResumeEpoch = 0, device="cuda", plot_in_train=False, save_images_folder = None, samples_tar = None, num_classes=None, num_features = 2):
    netG = netG.to(device)
    netD = netD.to(device)
    ...
    for epoch in range(ResumeEpoch, EPOCHS_GAN):
        ...
        if plot_in_train and (epoch+1)%50==0:
            ...
            z = torch.randn(samples_tar.shape[0], GAN_Latent_Length, dtype=torch.float).to(device)
            labels = np.random.choice(np.arange(num_classes), size=samples_tar.shape[0], replace=True)
