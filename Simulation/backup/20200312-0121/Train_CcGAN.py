...
def train_CcGAN(kernel_sigma, threshold_type, kappa, epoch_GAN, dim_GAN, trainloader, netG, netD, optimizerG, optimizerD, save_images_folder, save_models_folder = None, ResumeEpoch = 0, device="cuda", plot_in_train=False, samples_tar = None, angle_grid = None):
    '''
    ...
    for epoch in range(ResumeEpoch, epoch_GAN):
        for batch_idx, (batch_train_samples, batch_train_angles) in enumerate(trainloader):
            ...
            BATCH_SIZE = int(batch_train_samples.shape[0]/2)
            ...
            if threshold_type == "soft":
                real_weights_x_j = np.clip(np.exp(-kernel_sigma*(batch_train_angles_1.cpu().numpy()-batch_train_angles_2.cpu().numpy()-batch_epsilons_2)**2), 1e-20, 1e+20)
                fake_weights_x_j = np.clip(np.exp(-kernel_sigma*(batch_train_angles_1.cpu().numpy()+batch_epsilons_1-batch_train_angles_2.cpu().numpy()-batch_epsilons_2)**2), 1e-20, 1e+20)
            else:
        ...
        if plot_in_train and (epoch+1) % 50 == 0:
            ...
            z = torch.randn(samples_tar.shape[0], dim_GAN, dtype=torch.float).to(device)
            labels = np.random.choice(angle_grid/(2*np.pi), size=samples_tar.shape[0], replace=True)
            # labels = np.random.choice(angle_grid, size=samples_tar.shape[0], replace=True)
            ...
            ScatterPoints(samples_tar, prop_samples, filename)
