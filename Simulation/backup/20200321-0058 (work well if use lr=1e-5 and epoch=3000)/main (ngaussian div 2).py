...
parser.add_argument('--n_gaussians', type=int, default=120,
                    help = "Number of Gaussians in this experiment (number of angles's).") #half of them will be used for training and the rest are for testing
parser.add_argument('--max_n_samp_per_gaussian_train', type=int, default=10) # n_gaussians_train*n_rsamp_per_gaussian = ntrain
...
parser.add_argument('--n_samp_per_gaussian_eval', type=int, default=1000) # number of fake samples for each Gaussian
...
n_gaussians_all = args.n_gaussians
n_features = 2 # 2-D
radius = args.radius
# all angles
angle_grid_all = np.linspace(0, 2*np.pi, n_gaussians_all+1)
angle_grid_all = angle_grid_all[0:n_gaussians_all]
indx_all = np.arange(n_gaussians_all)
# angles for training
indx_train = indx_all[indx_all%2==0]
angle_grid_train = angle_grid_all[indx_train]
n_gaussians_train = len(angle_grid_train)
# angles for evaluation
indx_eval = np.setdiff1d(indx_all, indx_train)
assert len(np.intersect1d(indx_eval, indx_train))==0
angle_grid_eval = angle_grid_all[indx_eval]
n_gaussians_eval = len(angle_grid_eval)
...
for i in range(args.n_gaussians_plot):
    ...
    unseen_angle_grid_plot[i] = np.quantile(angle_grid_eval, quantile_i, interpolation='nearest')
...
gaussian_inflate_factor = 0

# ### threshold to determine high quality samples
# quality_threshold = sigma_gaussian*4 #good samples are within 7 standard deviation
...
def generate_data(max_n_samp_per_gaussian, angle_grid, fix_samp_per_gaussian=False):
    return sampler_CircleGaussian(max_n_samp_per_gaussian, angle_grid, radius = radius, fix_samp_per_gaussian=fix_samp_per_gaussian, sigma = sigma_gaussian, inflate_factor=gaussian_inflate_factor, dim = n_features)
...
for nSim in range(args.nsim):
    ...
    ###############################################################################
    samples_train, angles_train, means_train = generate_data(args.max_n_samp_per_gaussian_train, angle_grid_train) #this angles_train is not normalized; normalize if args.GAN is not cGAN.
    samples_plot_in_train, _, _ = generate_data(10, unseen_angle_grid_plot, fix_samp_per_gaussian=True)
    ...
    if args.GAN == "cGAN": #treated as classification; convert angles to class labels
        ...
        if args.kernel_sigma<0:
            ...
            # args.kernel_sigma = 1/n_gaussians_train

        if args.kappa < 0:
            if args.threshold_type=="hard":
                # args.kappa = 1/n_gaussians_train
                args.kappa = 2/n_gaussians_train
                # args.kappa = (1/n_gaussians_train + 3*args.kernel_sigma) * (len(angles_train)/n_gaussians_train)**(-1/2)
            else:
                # args.kappa = 1/((1/n_gaussians_train)**2)
                args.kappa = 1/((2/n_gaussians_train)**2)
    ...
    # cGAN: treated as a classification dataset
        ...
        Filename_GAN = save_models_folder + '/ckpt_' + args.GAN + '_epoch_' + str(args.epoch_gan) + '_SEED_' + str(args.seed) + '_nSim_' + str(nSim)
        if not os.path.isfile(Filename_GAN):
            netG = cond_generator(ngpu=NGPU, nz=dim_GAN, out_dim=n_features, num_classes=n_gaussians_train)
            netD = cond_discriminator(ngpu=NGPU, input_dim = n_features, num_classes=n_gaussians_train)

            criterion = nn.BCELoss()
            optimizerG = torch.optim.Adam(netG.parameters(), lr=lr_GAN, betas=(gan_Adam_beta1, gan_Adam_beta2))
            optimizerD = torch.optim.Adam(netD.parameters(), lr=lr_GAN, betas=(gan_Adam_beta1, gan_Adam_beta2))
            # Start training
            netG, netD, optimizerG, optimizerD = train_cGAN(epoch_GAN, dim_GAN, train_dataloader, netG, netD, optimizerG, optimizerD, criterion, save_models_folder = save_models_folder, ResumeEpoch = args.resumeTrain_gan, device=device, plot_in_train=plot_in_train, save_images_folder = save_GANimages_InTrain_folder, samples_tar_eval = samples_plot_in_train, angle_grid_eval = unseen_angle_grid_plot, num_classes=n_gaussians_train, unique_labels=unique_angles_train, label2class=angle2class, fig_size=fig_size, point_size=point_size)
            ...
            print("Loading pre-trained generator >>>")
            checkpoint = torch.load(Filename_GAN)
            netG = cond_generator(ngpu=NGPU, nz=dim_GAN, out_dim=n_features, num_classes=n_gaussians_train).to(device)
            netG.load_state_dict(checkpoint['netG_state_dict'])

        # function for sampling from a trained GAN
        def fn_sampleGAN(nfake, batch_size):
            fake_samples, _ = SampcGAN(netG, class2label=class2angle, GAN_Latent_Length = dim_GAN, NFAKE = nfake, batch_size = batch_size, num_classes = n_gaussians_train, device=device)
            return fake_samples
    ...
    if args.eval:
        ...
        for i_ang in tqdm(range(len(angle_grid_eval))):
            ...
            real_cov_curr = np.eye(n_features)*(sigma_gaussian*(1+gaussian_inflate_factor*angle_curr/(2*np.pi)))**2 #covraiance matrix for each Gaussian
            ...
            two_w_dist_curr = two_wasserstein(real_mean_curr, fake_mean_curr, real_cov_curr, fake_cov_curr)
        ...
        real_samples_plot, _, _ = generate_data(args.n_samp_per_gaussian_plot, unseen_angle_grid_plot, fix_samp_per_gaussian=True)
