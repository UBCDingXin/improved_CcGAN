...
class cont_cond_generator(nn.Module):
    def __init__(self, ngpu=1, nz=2, out_dim=2):
        ...
        self.linear = nn.Sequential(
                nn.Linear(nz+1, self.inner_dim, bias=set_bias),
...
class cont_cond_discriminator(nn.Module):
    def __init__(self, ngpu=1, input_dim = 2):
        ...
        self.main = nn.Sequential(
            nn.Linear(input_dim+1, self.inner_dim, bias=set_bias),
            # nn.BatchNorm1d(self.inner_dim),
            nn.ReLU(True),
            nn.Linear(self.inner_dim, self.inner_dim, bias=set_bias),
            # nn.BatchNorm1d(self.inner_dim),
        ...
        self.out = nn.Sequential(
            nn.Linear(self.inner_dim+1, 1, bias=set_bias), #layer 4
            nn.Sigmoid()
        )
    ...
    def forward(self, input, labels):
        ...
        if input.is_cuda and self.ngpu > 1:
            ...
            output = nn.parallel.data_parallel(self.out, output, range(self.ngpu))
        else:
