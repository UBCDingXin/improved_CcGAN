...
def train_cGAN(images, labels, netG, netD, save_images_folder, save_models_folder = None):
    ...
    if save_models_folder is not None and resume_niters>0:
        save_file = save_models_folder + "/cGAN-concat_checkpoint_intrain/cGAN-concat_checkpoint_niters_{}.pth".format(resume_niters)
    ...
    for niter in range(resume_niters, niters):
        ...
        if (niter+1)%100 == 0:
            print ("cGAN-concat: [Iter %d/%d] [D loss: %.4f] [G loss: %.4f] [D prob real:%.4f] [D prob fake:%.4f] [Time: %.4f]" % (niter+1, niters, d_loss.item(), g_loss.item(), prob_real.mean().item(),prob_fake.mean().item(), timeit.default_timer()-start_time))
        ...
        if save_models_folder is not None and ((niter+1) % save_niters_freq == 0 or (niter+1) == niters):
            save_file = save_models_folder + "/cGAN-concat_checkpoint_intrain/cGAN-concat_checkpoint_niters_{}.pth".format(niter+1)
...
def SampcGAN_given_label(netG, given_label, NFAKE = 10000, batch_size = 500):
    ...
    fake_labels = np.ones(NFAKE) * given_label #use assigned label
